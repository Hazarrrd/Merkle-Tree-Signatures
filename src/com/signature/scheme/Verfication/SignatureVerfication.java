...
public class SignatureVerfication {
    ...
    public static Boolean verifySignature (PublicKey publicKey, Signature signature,PseudorndFunction f, int l1L, int l2L,int l1U, int l2U, int wL,int wU, byte[] x, byte[] m,int heightL, int heightU){
        ...
        Node node = MTreeOperations.leafCalc(n,l,OTSpublicKey,publicKey.bitmaskLTree,index);
        ...
        for(int i =1;i<=heightL;i++){
            if(Math.floor(index/Math.pow(2,i)) % 2 == 0){
                node = MTreeOperations.computeParent(node,authL[i-1],bitmask[i]);
            } else {
                node = MTreeOperations.computeParent(authL[i-1],node,bitmask[i]);
            }
        }
        ...
        //Checking if publicKey belongs to Merkle Tree
        node = MTreeOperations.leafCalc(n,l,OTSpublicKey,publicKey.bitmaskLTree,index);
        Node[] authU = signature.upperAuthPath;

        for(int i =1;i<=heightU;i++){
            if(Math.floor(index/Math.pow(2,i)) % 2 == 0){
                node = MTreeOperations.computeParent(node,authU[i-1],bitmask[i]);
            } else {
                node = MTreeOperations.computeParent(authU[i-1],node,bitmask[i]);
            }
        }
